...
export async function emailReport(loanId: string, recipientEmail?: string): Promise<void> {
  const res = await fetch(`${SCORING_SERVICE_URL}/report/${loanId}/email`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!res.ok) {
    ...
    throw new Error(error?.detail || "Failed to email report");
  }
}
